  ...
  const acceptBooking = async () => {
    try {
      ...
      if (response.statusCode === 200) {
        // Get confirmation code from booking object
        const code = response.booking?.bookingConfirmationCode || 'PENDING';
        setConfirmationCode(code); // Set booking confirmation code
        setShowMessage(true); // Show message
        // Hide message and navigate to profile after 10 seconds
        setTimeout(() => {
          setShowMessage(false);
          navigate('/profile'); // Navigate to profile to see booking
        }, 10000);
      }
    } catch (error) {
    ...
    <div className="room-details-booking">
      {showMessage && (
        <div className="booking-success-message" style={{
          ...
          <h3 style={{color: '#28a745', marginTop: 0}}>✓ Booking Successful!</h3>
          <p style={{fontSize: '16px', marginBottom: '10px'}}>
            <strong>Confirmation Code:</strong> <span style={{color: '#ff9800', fontSize: '20px', fontWeight: 'bold'}}>{confirmationCode}</span>
          </p>
          <p style={{fontSize: '14px', color: '#666'}}>
            📧 Booking confirmation details have been sent to your email.
          </p>
          <p style={{fontSize: '14px', color: '#666'}}>
            💰 <strong>Payment Method:</strong> Cash on check-in
          </p>
          <p style={{fontSize: '14px', color: '#666'}}>
            📱 Please save your confirmation code for reference.
          </p>
          <p style={{fontSize: '13px', color: '#999', marginTop: '15px'}}>
            Redirecting to rooms in 10 seconds...
          </p>
        </div>
      )}
      {errorMessage && (
